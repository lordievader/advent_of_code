...
fn op_input(input: &Vec<i32>, index: usize, value: i32) -> (i32, Vec<i32>, i32) {
    let mut output = input.to_vec();
    let outcome_location = input[(index + 1) as usize];
...
pub fn execute(input: &Vec<i32>, mut input_value: Vec<i32>) -> (i32, Vec<i32>) {
    ...
    loop {
        ...
        let result = match opcode {
            ...
            3 => op_input(&output, index, input_value.pop().unwrap()),
        ...
        output = result.1;
        // print!("Jumping: {} + {} = ", index, result.2);
        index = (index as i32 + result.2) as usize;
        // println!("{}", index);
        if index >= length {
            break;
        }
...
pub fn get_output(input: &Vec<i32>, start_index: usize, mut input_value: Vec<i32>, mut output_value: i32) -> Amp {
    let length = input.len();
    let mut output = input.to_vec();
...
mod tests{
    ...
    #[test]
    fn test_execute(){
        assert_eq!(execute(&vec![1002, 4, 3, 4, 33], vec![0]), (0, vec![1002, 4, 3, 4, 99]));
        assert_eq!(execute(&vec![1, 0, 0, 0, 99], vec![5]), (0, vec![2, 0, 0, 0, 99]));
        assert_eq!(execute(&vec![2, 3, 0, 3, 99], vec![5]), (0, vec![2, 3, 0, 6, 99]));
        assert_eq!(execute(&vec![2, 4, 4, 5, 99, 0], vec![5]), (0, vec![2, 4, 4, 5, 99, 9801]));
        assert_eq!(execute(&vec![1, 1, 1, 4, 99, 5, 6, 0, 99], vec![5]), (0, vec![30, 1, 1, 4, 2, 5, 6, 0, 99]));
        assert_eq!(execute(&vec![3, 0, 4, 0, 99], vec![5]), (5, vec![5, 0, 4, 0, 99]));

        assert_eq!(
            execute(&vec![3,12,6,12,15,1,13,14,13,4,13,99,-1,0,1,9], vec![0]),
