...
def build_grid(problem_input):
    ...
    for row, line in enumerate(problem_input.split('\n')):
        ...
        while found is True:
            if '^' in line:
                column = line.index('^')
                carts.append(Cart(up, row, column))
                line = f"{line[:column]}|{line[column+1:]}"

            elif 'v' in line:
                column = line.index('v')
                carts.append(Cart(down, row, column))
                line = f"{line[:column]}|{line[column+1:]}"

            elif '<' in line:
                column = line.index('<')
                carts.append(Cart(left, row, column))
                line = f"{line[:column]}-{line[column+1:]}"

            elif '>' in line:
                column = line.index('>')
                carts.append(Cart(right, row, column))
                line = f"{line[:column]}-{line[column+1:]}"
...
def crash(carts):
    ...
    return crashed, cart_a, cart_b

def update_cart_list(carts):
    ...
    :return: carts
    """
    crashed, cart_a, cart_b = crash(carts)
    while crashed is True:
        carts.remove(cart_a)
        carts.remove(cart_b)
        crashed, cart_a, cart_b = crash(carts)
...
def solution(problem_input):
    ...
    while len(carts) > 1:
        ...
        carts = update_cart_list(carts)
        # print_grid(grid, carts)

    position = numpy.flip(carts[0].position, axis=1)
